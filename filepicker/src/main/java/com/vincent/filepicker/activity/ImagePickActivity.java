...
public class ImagePickActivity extends BaseActivity {
    ...
    private void checkCamera() {
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (checkCallPhonePermission != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.CAMERA}, 3366);
            }
        }
    }
        ...
        switch (requestCode) {
            case Constant.REQUEST_CODE_TAKE_IMAGE:
                if (resultCode == RESULT_OK) {
                    ...
                    File file = new File(ImagePickAdapter.mImagePath);
                    ...
                    getApplicationContext().getContentResolver().delete(ImagePickAdapter.mImageUri, null, null);
                }
                break;
    ...
    private void refreshData(List<Directory<ImageFile>> directories) {
        ...
        if (tryToFindTakenImage && !TextUtils.isEmpty(ImagePickAdapter.mImagePath)) {
            File takenImageFile = new File(ImagePickAdapter.mImagePath);
    ...
    private boolean findAndAddTakenImage(List<ImageFile> list) {
        for (ImageFile imageFile : list) {
            if (imageFile.getPath().equals(ImagePickAdapter.mImagePath)) {
